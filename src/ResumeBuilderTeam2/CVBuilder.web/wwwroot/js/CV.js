$(document).ready(function () {
    ...
    $('#test').on('DOMSubtreeModified', function () {
        
       
        const mainCV = document.getElementById('test');
        mainCV.setAttribute('contenteditable', 'false');

        const cvContent = mainCV.innerHTML;

        // Create a new temporary div element to work with the HTML string as a DOM element
        const tempDiv = document.createElement('div');
        tempDiv.innerHTML = cvContent;

        const deleteSectionElements = tempDiv.querySelectorAll('.delete-section');
        deleteSectionElements.forEach(element => {
            element.remove();
        });

        const deleteInfoElements = tempDiv.querySelectorAll('.delete-info');
        deleteInfoElements.forEach(element => {
            element.remove();
        });

        // Remove the "btn-cont" element and its child elements
        const btnCont = tempDiv.querySelector('.btn-cont');
        if (btnCont) {
            btnCont.remove();
        }

        const cvPreview = document.getElementById('cvPreview');
        cvPreview.innerHTML = tempDiv.innerHTML;
    ...
    $('#heading-btn').click(function () {
        const invoice = document.getElementById("CV");
        ...
        $("#main-CV").addClass('download-active');
        $("#contact-section").addClass('download-active');
        html2pdf().from(invoice).set(opt).save();
    });

    $('#edit-btn').click(function () {
        $('#main-CV').removeClass('download-active');
    ...
    $('#add-smpro-btn').click(function () {
        document.getElementById('ref-list').innerHTML += `<li class="skill-item">
        <div class="skill-text"><span contenteditable="true"
                style='font-weight: 600;'>Name</span>: <span contenteditable="true">Role</span>
        </div>
        <div class="delete-info fa fa-trash" onclick="deleteFromList(this)"></div>
    </li>`;
    });
    ...
    $('#add-edu-btn').click(function () {
        document.getElementById('edu-list').innerHTML += `<li class="content-item">
            ...
            <ul class="content-item-desc-list">
                <li class="content-item-desc-item"><div class='content-desc-text' contenteditable="true">Academic Descriptions</div><div class="delete-info fa fa-trash" onclick="deleteFromList(this)"></div></li>
                <li class="content-item-desc-item"><div class='content-desc-text' contenteditable="true">Academic Descriptions</div><div class="delete-info fa fa-trash" onclick="deleteFromList(this)"></div></li>
                <li class="content-item-desc-item"><div class='content-desc-text' contenteditable="true">Academic Descriptions</div><div class="delete-info fa fa-trash" onclick="deleteFromList(this)"></div></li>
            </ul>
        </li>`;
    ...
    $('#add-job-btn').click(function () {
        document.getElementById('job-list').innerHTML += `<li class="content-item">
            ...
            <div class="brief-cont">
                <div class="content-item-role" contenteditable="true">Your position</div>
                <div class="content-item-duration"><span contenteditable="true">from</span> - <span contenteditable="true">to</span></div>
            </div>
